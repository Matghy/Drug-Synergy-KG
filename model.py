...
class KGANS(nn.Module):
    ...
    def  __init__(self,args, n_entitys, n_relations, e_dim, r_dim,
        ...
        self.relation_embs = nn.Embedding(n_relations, r_dim)
    ...
    def _init_weight(self):
        # Load custom initialization values
        drug_init_embs = pd.read_csv('drug_init_embs.csv').values[:, 3:]  # Assuming 0-based indexing
        init_copy_number = pd.read_csv('init_copy_number.csv').values[:, 2:]  # Assuming 0-based indexing

        # Custom initialization for the first part of the embeddings
        self.entity_embs.weight.data[:763] = torch.tensor(drug_init_embs, dtype=torch.float32)
        self.entity_embs.weight.data[763:840] = torch.tensor(init_copy_number, dtype=torch.float32)

        # Xavier uniform initialization for the rest
        nn.init.xavier_uniform_(self.entity_embs.weight.data[840:])
        nn.init.xavier_uniform_(self.relation_embs.weight)
